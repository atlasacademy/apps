...
const warFaq = (region: Region) => {
    const artificialIntelligence = (
        <>
            Aside from user actions, battles are a product of enemy and field artificial intelligence.
            They control events ranging from debuffs at the start of a quest or enemies using an invuln pierce right before using their noble phantasm.
            Ai is a sequence of attack and skills that is decided by a mix of conditions being fulfilled and randomness.
            ...
            <br></br>
            <br></br>
            <img alt="Ai Basic Example" src={ai_basic_example} width="100%"></img>
            <br></br>

            The artificial intelligence can be represented by a graph like at the top of ai pages (<Link to={`/${region}/ai/svt/94035321`}>example</Link>).
             The graph is made out of nodes and edges. The nodes with bold text represent the parent ai while the others are their sub ai. The parent node is connected to its 
             childs with full arrows. Which sub ai is chosen will depend on different factors such as conditions and randomness. A common example is the ai using defensive
             skills like evade or guts - they usually need to be under a certain hp threshold. The sub ai will then go to another parent ai with a dotted line or go to its
             own parent. Lastly, enemies are not tied to a single ai. For instance, enemies with break bars have an ai for each bar. Simple enemies will have basic graphs while
             bosses' will be much more complex.
        </>
    );

    const aiSubId = (
        <>
            Each parent ai (for example 94035323) are divived into sub ids. Once the ai reaches that parent ai, it will then need to decide which sub ai to go to. Some ai may
            contain one or few sub ai and reaching the sub ai will be fairly straightforward. On the other hand, some parent ai have a lot of sub ai and the process to finding
            the next child ai is more complex.
        </>
    );

    const nextAi = (
        <>
            Once the action in the sub ai has been completed, it will go to the next parent ai which is illustrated with a dotted line on the graph. This parent ai is denoted
            in <b>Next AI</b>. In some cases, the <b>Next Ai</b> section will be empty, this means the next parent ai is its own parent. The process will then continue until
    ...
    const actNum = (
        <>
            <b>Act Num</b> ia an identifier to when the ai is used. For example, you have <b>Reaction Enemyturn Start</b> and <b>Reaction Enemyturn End</b>, the beginning and
            ...
            actions. However, the Act Num <b>Anytime</b> does, it's the one used for regular attack and skills. Lastly,
            <b>Max Np</b> is used right before the enemy uses its noble phantasm to use certain skills like pierce invuln.
            <br></br>
    ...
    const condition = (
        <>
            This is a prerequisite factor that needs to be true in order for the sub ai to be executed. While there a ton of different possible conditions,
    ...
    const priorityWeight = (
        <>
            This represents the order in which the sub ai are considered and the weight if there are multiple sub ai in that priority bracket.
            All of the sub ai are sorted based on their priority from high to low. Let's take these priorities, <b>100, 50, 50, 50, 1</b>. The sub ai with the highest
            priority is 100 and is alone in its bracket. If its condition is reached, it will always be executed. Next up are the three sub ai with a priority of 50.
            First, all of these sub ai's condition will be checked. If none succeed, it will go to the remaining sub ai. If more than one succeed, their weight will
            be used. The weight of each viable ai are added up and into a sum - with a weight of 30, 50 and 60 respectively, the odds of each one happening would be
    ...
    const actTarget = (
        <>
            ...
            <br></br><br></br>
            An interesting case study of ai targetting is Kirschtaria Wodime from <Link to={`/${region}/quest/3000515/3`}>Atlantis 11-3</Link> demonstrated in this
