...
export const useClassBoardMapCanvas = (options: UseClassBoardMapOptions) => {
    const { classBoard, currentSquare, hoveredSquareId, squareImages, zoom, panX, panY } = options;
        ...
        (ctx: CanvasRenderingContext2D, canvas: HTMLCanvasElement) => {
            ...
            classBoard.squares.forEach((square: ClassBoard.ClassBoardSquare) => {
                ...
                if (isSelected) {
                    ...
                    ctx.fillRect(
                        x - size / 2 - SQUARE_SELECTED_PADDING,
                        y - size / 2 - SQUARE_SELECTED_PADDING,
                        size + SQUARE_SELECTED_PADDING * 2,
                        size + SQUARE_SELECTED_PADDING * 2
                    );
                } else if (isHovered) {
                    ctx.fillStyle = SQUARE_HOVERED_COLOR;
                    ctx.fillRect(
                        x - size / 2 - SQUARE_HOVERED_PADDING,
                        y - size / 2 - SQUARE_HOVERED_PADDING,
                        size + SQUARE_HOVERED_PADDING * 2,
                        size + SQUARE_HOVERED_PADDING * 2
                    );
                }
                ...
                ctx.strokeStyle = isSelected
                    ...
                    : isHovered
                      ? SQUARE_HOVERED_BORDER
                      : SQUARE_DEFAULT_BORDER;
                ctx.lineWidth = isSelected
                    ? SQUARE_SELECTED_BORDER_WIDTH
                    : isHovered
                      ? SQUARE_HOVERED_BORDER_WIDTH
                      : SQUARE_DEFAULT_BORDER_WIDTH;
