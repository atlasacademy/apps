...
import {artoria, musashi, musashiSummer} from "../../helpers";
...
describe('BattleServantActor hits', () => {
    ...
    it('musashi skill 1', async () => {
        ...
        expect(servant.hits(actions.get(1))).to.eql([3, 7, 10, 14, 17, 21, 28]);
    ...
    it('musashi summer skill 2', async () => {
        let battle = new Battle(null),
            servant = musashiSummer(BattleTeam.PLAYER),
            actions: BattleAttackActionList;
        ...
        expect(servant.hits(actions.get(1))).to.eql([10, 20, 30, 40]);
        expect(servant.hits(actions.get(2))).to.eql([10, 20, 30, 40]);
        expect(servant.hits(actions.get(3))).to.eql([16, 16, 33, 33, 51, 51]);
        expect(servant.hits(actions.get(4))).to.eql([6, 13, 20, 26, 35]);
    });
});
