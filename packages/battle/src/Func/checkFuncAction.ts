...
export default async function checkFuncAction(battle: Battle,
                                              func: BattleFunc,
                                              actor: BattleActor,
                                              target: BattleActor): Promise<boolean> {
    ...
    switch (func.props.func.funcType) {
        ...
        case Func.FuncType.SUB_STATE:
            const traits = func.props.func.traitVals,
                resistMagnification = target.buffs().netBuffsRate(
                    Buff.BuffAction.TOLERANCE_SUBSTATE,
                    target.traits(target.buffs().traits(true)),
                    actor.traits(traits)
                ),
                successMagnification = actor.buffs().netBuffsRate(
            ...
            chance = Variable
                .float(rand)
                .add(Variable.float(resistMagnification))
                .cast(VariableType.INT)
                .value();
            rate = Variable
                .float(baseRate)
                .add(Variable.float(successMagnification))
                .cast(VariableType.INT)
                .value();
            break;
