...
async function checkBuffSuccess(battle: Battle,
                                func: BattleFunc,
                                buff: BattleBuff,
                                actor: BattleActor,
                                target: BattleActor): Promise<boolean> {
    let baseRate = func.state.dataVal.Rate ?? 0,
        buffResist = target.buffs().netBuffs(
            Buff.BuffAction.RESISTANCE_STATE,
            target.traits(),
            actor.traits(buff.traits())
        ),
        buffChanceBonus = actor.buffs().netBuffs(
            Buff.BuffAction.GRANT_STATE,
            actor.traits(buff.traits()),
            target.traits()
        ),
