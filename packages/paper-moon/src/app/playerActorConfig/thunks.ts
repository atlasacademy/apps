...
export const playerActorConfigAddThunk = (): AppThunk => {
    return async (dispatch, getState) => {
        ...
        if (!state.playerActorConfig.ready)
            return;
...
export const playerActorConfigOpenServantThunk = (id: number): AppThunk => {
    return async dispatch => {
        ...
        await dispatch(playerActorConfigSlice.actions.setServantOptions({
            name: '',
            level: '',
        }));
        ...
        await dispatch(playerActorConfigSlice.actions.setDefaultServantOptions({
            level: servant.lvMax,
            name: servant.name,
        }));
        await dispatch(playerActorConfigSlice.actions.setServantOptions({
...
export const playerActorConfigValidateServantOptions = (): AppThunk => {
    return async (dispatch, getState) => {
        ...
        if (!servantOptions.name)
            servantOptions.name = defaultServantOptions.name;

        servantOptions.level = parseInt(servantOptions.level.toString());
        if (isNaN(servantOptions.level)
