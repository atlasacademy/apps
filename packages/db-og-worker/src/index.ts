...
function overwrite(_ : Response, overwrite_ : string) {
    return new HTMLRewriter().on('[property="og:title"]', new Handler(overwrite_)).transform(_);
}
...
async function handleRequest(request : Request) {
    ...
    let [region, subpage, target, ...paths] = pathname.split('/').filter(Boolean);
    ...
    switch (subpage) {
        case `servant`: {
            let { name } = await fetch(`https://api.atlasacademy.io/basic/${region}/servant/${target}?lang=en`).then(res => res.json());
            return overwrite(page, `[${region}] Servant - ${name} - Atlas Academy DB`);
        }
        default:
