...
export default function (func: Func.Func, dataVal: DataVal.DataVal): BasePartial[] {
    ...
    switch (func.funcType) {
        ...
        case Func.FuncType.DAMAGE_NP_INDIVIDUAL_SUM:
            ...
            addPartials(partials, [
                new ParticlePartial('('),
                new TextPartial('bonus per trait'),
            ]);
        ...
        case Func.FuncType.DAMAGE_NP_RARE:
            ...
            (dataVal.TargetRarityList ?? []).forEach(rarity => {
                addPartials(rarityDescriptions, [
                    new ValuePartial(ValueType.NUMBER, rarity)
                ], '/');
            });

            if (rarityDescriptions.length === 1)
                addPartials(rarityDescriptions, [new TextPartial('rarity')]);
            else
                addPartials(rarityDescriptions, [new TextPartial('rarities')]);

            addPartials(partials, [
                new ParticlePartial('('),
                new TextPartial('bonus to'),
            ]);
        ...
        case Func.FuncType.ENEMY_ENCOUNT_RATE_UP:
            if (dataVal.Individuality)
                addPartials(partials, [
                    new ParticlePartial('with '),
                    new TraitReferencePartial(dataVal.Individuality)
                ]);
    ...
    if (func.functvals.length) {
        addPartials(partials, [
            new ParticlePartial('if '),
            new TextPartial('on field'),
            new ParticlePartial(' '),
        ]);
